...
is it a syntax error? if yes:
open les yeux
is it semantic error? if yes:
cry about it

# Iteration
quest'que ce une method de string?
une method de string est une function qui travailler avec une string
par examples, .lower()

.strip() removes chars from beginning-end of string
whatever.strip("insert stuff to delete")
# Iteration
